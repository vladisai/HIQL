...
flags.DEFINE_float("high_p_randomgoal", 0.0, "")
...
def main(_):
    ...
    FLAGS.config["value_hidden_dims"] = (
        FLAGS.value_hidden_dim,
    ) * FLAGS.value_num_layers
    ...
    if "antmaze" in FLAGS.env_name:
        ...
        if FLAGS.env_name == "procgen-500":
            dataset = get_procgen_dataset(
                "data/procgen/level500.npz", state_based=("state" in FLAGS.env_name)
            )
            min_level, max_level = 0, 499
        elif FLAGS.env_name == "procgen-1000":
            dataset = get_procgen_dataset(
                "data/procgen/level1000.npz", state_based=("state" in FLAGS.env_name)
            )
        ...
        goal_infos.append(
            {
                "eval_level": [
                    level for level in large_levels if min_level <= level <= max_level
                ],
        ...
        goal_infos.append(
            {
                "eval_level": [level for level in large_levels if level > max_level],
                "eval_level_name": "test",
            }
        )
    ...
    for i in tqdm.tqdm(range(1, total_steps + 1), smoothing=0.1, dynamic_ncols=True):
        ...
        if i == 1 or i % FLAGS.eval_interval == 0:
            ...
            base_observation = jax.tree_map(
                lambda arr: arr[0], pretrain_dataset.dataset["observations"]
            )
            if "procgen" in FLAGS.env_name:
                ...
                for goal_info in goal_infos:
                    ...
                    eval_metrics.update(
                        {
                            f'evaluation/level{goal_info["eval_level_name"]}_{k}': v
