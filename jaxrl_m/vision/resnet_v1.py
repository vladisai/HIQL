...
class SpatialSoftmax(nn.Module):
    ...
    def __call__(self, feature):
        ...
        feature = feature.transpose(0, 3, 1, 2).reshape(
...
class ResNetEncoder(nn.Module):
    ...
    def __call__(self, observations: jnp.ndarray, train: bool = True, cond_var=None):
        ...
        conv = partial(
            self.conv, use_bias=False, dtype=self.dtype, kernel_init=kaiming_init()
        )
        if self.norm == "batch":
            norm = partial(
                nn.BatchNorm,
        ...
        for i, block_size in enumerate(self.stage_sizes):
            for j in range(block_size):
                stride = (
                    (strides[i + 1], strides[i + 1]) if i > 0 and j == 0 else (1, 1)
                )
                x = self.block_cls(
                ...
                if self.use_multiplicative_cond:
                    ...
                    cond_out = nn.Dense(x.shape[-1], kernel_init=xavier_init())(
                        cond_var
                    )
