...
def evaluate_with_trajectories(
    ...
    for i in range(num_episodes + num_video_episodes):
        ...
        if "procgen" in env_name:
            ...
            border_states = [
                i
                for i in range(len(level_details.locs))
                if len(
                    [
                        1
                        for j in range(len(level_details.locs))
                        if abs(level_details.locs[i][0] - level_details.locs[j][0])
                        + abs(level_details.locs[i][1] - level_details.locs[j][1])
                        < 7
                    ]
                )
                <= 2
        ...
        while not done:
            if not use_waypoints:
                ...
                if config["use_rep"]:
                    cur_obs_goal_rep = policy_rep_fn(
                        targets=cur_obs_goal, bases=observation
                    )
                else:
                    cur_obs_goal_rep = cur_obs_goal
            else:
                cur_obs_goal = high_policy_fn(
                ...
                if config["use_rep"]:
                    cur_obs_goal = (
                        cur_obs_goal
                        / np.linalg.norm(cur_obs_goal, axis=-1, keepdims=True)
                        * np.sqrt(cur_obs_goal.shape[-1])
                    )
                else:
            ...
            if "procgen" in env_name:
                ...
                cur_frame[
                    2,
                    goal_loc[1] - 1 : goal_loc[1] + 2,
                    goal_loc[0] - 1 : goal_loc[0] + 2,
                ] = 255
                cur_frame[
                    :2,
                    goal_loc[1] - 1 : goal_loc[1] + 2,
                ...
                if i >= num_episodes and step % 3 == 0:
                    if "antmaze" in env_name:
                        ...
                        if (
                            use_waypoints
                            and not config["use_rep"]
                            and ("large" in env_name or "ultra" in env_name)
                        ):
