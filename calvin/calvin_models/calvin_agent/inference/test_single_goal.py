...
def parse_reprod_act_vid():
    eval_filename = (
        "./data/validation/friday_microwave_topknob_bottomknob_slide_0_path.pkl"
    )
    demo_files = [
    ...
    for name in demo_files:
        ...
        reproduce_file_actions(
            file_path, show_video=False, save_video=True, save_filename=video_name
        )
...
def test(
    ...
    for goal, name in zip(goals, names):
        for i in range(n_runs):  # save n_runs videos for each goal
            ...
            video_name = (
                "%s_npf_%d_" % (model_name, sample_new_plan) + name + "(%d).mp4" % i
            )
            test_model(
                model,
                goal_file_path,
...
if __name__ == "__main__":
    ...
    goals = [
        ...
        "bottomknob",
        "grip_hinge",
        "subsequent/microwave_bottomknob_topknob",
    ]
