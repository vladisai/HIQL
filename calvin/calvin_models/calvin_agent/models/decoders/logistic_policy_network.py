...
class LogisticPolicyNetwork(ActionDecoder):
    def __init__(
        ...
        self._setup_action_bounds(
            dataset_dir, act_max_bound, act_min_bound, load_action_bounds
        )
        # hack for mypy
    ...
    def loss_and_act(
        ...
        logit_probs, log_scales, means, _ = self(
    ...
    def _setup_action_bounds(
        ...
        if load_action_bounds:
            try:
                statistics_path = (
                    Path(calvin_agent.__file__).parent
                    / dataset_dir
    ...
    def forward(  # type: ignore
        ...
        latent_plan = (
            latent_plan.unsqueeze(1).expand(-1, seq_len, -1)
            if latent_plan.nelement() > 0
            else latent_plan
        )
