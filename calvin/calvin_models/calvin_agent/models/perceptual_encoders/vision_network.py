...
class VisionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_softmax = SpatialSoftmax(
            num_rows=w, num_cols=h, temperature=1.0
        )  # shape: [N, 128]
        # model
        self.conv_model = nn.Sequential(
            # input shape: [N, 3, 200, 200]
            nn.Conv2d(
                in_channels=num_c, out_channels=32, kernel_size=8, stride=4
            ),  # shape: [N, 32, 49, 49]
            self.act_fn,
            nn.Conv2d(
...
class SpatialSoftmax(nn.Module):
    def __init__(
        self, num_rows: int, num_cols: int, temperature: Optional[float] = None
    ):
        """
