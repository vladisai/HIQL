...
class ConcatEncoders(nn.Module):
    def __init__(
        ...
        self.vision_gripper_encoder = (
            hydra.utils.instantiate(vision_gripper) if vision_gripper else None
        )
    ...
    def forward(
        ...
        depth_static = (
            depth_imgs["depth_static"] if "depth_static" in depth_imgs else None
        )
        depth_gripper = (
            depth_imgs["depth_gripper"] if "depth_gripper" in depth_imgs else None
        )

        b, s, c, h, w = imgs_static.shape
        imgs_static = imgs_static.reshape(
            -1, c, h, w
        )  # (batch_size * sequence_length, 3, 200, 200)
        if depth_static is not None:
            depth_static = torch.unsqueeze(depth_static, 2)
            depth_static = depth_static.reshape(
                -1, 1, h, w
            )  # (batch_size * sequence_length, 1, 200, 200)
            imgs_static = torch.cat(
                [imgs_static, depth_static], dim=1
            )  # (batch_size * sequence_length, 3, 200, 200)
        # ------------ Vision Network ------------ #
        ...
        if imgs_gripper is not None:
            ...
            imgs_gripper = imgs_gripper.reshape(
                -1, c, h, w
            )  # (batch_size * sequence_length, 3, 84, 84)
            if depth_gripper is not None:
                depth_gripper = torch.unsqueeze(depth_gripper, 2)
                depth_gripper = depth_gripper.reshape(
                    -1, 1, h, w
                )  # (batch_size * sequence_length, 1, 84, 84)
                imgs_gripper = torch.cat(
                    [imgs_gripper, depth_gripper], dim=1
                )  # (batch_size * sequence_length, 4, 84, 84)
            encoded_imgs_gripper = self.vision_gripper_encoder(
                imgs_gripper
            )  # (batch*seq_len, 64)
            encoded_imgs_gripper = encoded_imgs_gripper.reshape(
                b, s, -1
            )  # (batch, seq, 64)
        ...
        if imgs_tactile is not None:
            ...
            imgs_tactile = imgs_tactile.reshape(
                -1, c, h, w
            )  # (batch_size * sequence_length, 3, 84, 84)
