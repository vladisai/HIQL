...
class Tasks:
    def __init__(self, tasks):
        ...
        self.tasks = {
            name: partial(getattr(self, args[0]), *args[1:])
            for name, args in dict(tasks).items()
        }
    ...
    def get_task_info_for_set(
        self, start_info: Dict, end_info: Dict, task_filter: Set
    ) -> Set:
        """
        start_info: dict with scene info and robot info
        end_info: dict with scene info and robot info
        ...
        return {
            task_name
            for task_name, function in self.tasks.items()
            if task_name in task_filter
            and function(start_info=start_info, end_info=end_info)
        }
    ...
    def rotate_object(
        ...
        if z_degrees > 0:
            return (
                z_degrees < z < z_threshold
                and abs(x) < x_y_threshold
                and abs(y) < x_y_threshold
            )
        else:
            return (
                z_degrees > z > -z_threshold
    ...
    def push_object(obj_name, x_direction, y_direction, start_info, end_info):
        ...
        start_contacts = set(
            (c[2], c[4]) for c in obj_start_info["contacts"] if c[2] != robot_uid
        )
        end_contacts = set(
            (c[2], c[4]) for c in obj_end_info["contacts"] if c[2] != robot_uid
        )

        # computing set difference to check if object had surface contact (excluding robot) at both times
        surface_contact = (
            len(start_contacts) > 0
            and len(end_contacts) > 0
            and start_contacts <= end_contacts
        )
    ...
    def lift_object(
        ...
        if surface_body and surface_link is None:
            ...
            surface_uid = start_info["scene_info"]["fixed_objects"][surface_body]["uid"]
            surface_link_id = start_info["scene_info"]["fixed_objects"][surface_body][
                "links"
            ][surface_link]
    ...
    def place_object(dest_body, dest_link=None, start_info=None, end_info=None):
        ...
        _obj_name = [
            k
            for k, v in start_info["scene_info"]["movable_objects"].items()
            if v["uid"] == obj_uid
        ]
        ...
        object_contacts_start = set(
            c[2]
            for c in start_info["scene_info"]["movable_objects"][obj_name]["contacts"]
        )
        if dest_link is None:
            object_contacts_end = set(
                c[2]
                for c in end_info["scene_info"]["movable_objects"][obj_name]["contacts"]
            )
            return (
            ...
            dest_link_id = end_info["scene_info"]["fixed_objects"][dest_body]["links"][
                dest_link
            ]
    ...
    def push_object_into(
        ...
        if isinstance(obj_name, (list, ListConfig)):
            return any(
                Tasks.push_object_into(
                    ob, src_body, src_link, dest_body, dest_link, start_info, end_info
                )
                for ob in obj_name
            )
        robot_uid = start_info["robot_info"]["uid"]
        ...
        src_link_id = start_info["scene_info"]["fixed_objects"][src_body]["links"][
            src_link
        ]
        dest_uid = end_info["scene_info"]["fixed_objects"][dest_body]["uid"]
        dest_link_id = end_info["scene_info"]["fixed_objects"][dest_body]["links"][
    ...
    def move_door_abs(joint_name, start_threshold, end_threshold, start_info, end_info):
        ...
        """
        start_joint_state = start_info["scene_info"]["doors"][joint_name][
            "current_state"
        ][0]
    ...
    def move_door_rel(joint_name, threshold, start_info, end_info):
        ...
        return (
            0 < threshold < end_joint_state - start_joint_state
            or 0 > threshold > end_joint_state - start_joint_state
        )

    @staticmethod
    def toggle_light(light_name, start_state, end_state, start_info, end_info):
        return (
            start_info["scene_info"]["lights"][light_name]["logical_state"]
            == start_state
            and end_info["scene_info"]["lights"][light_name]["logical_state"]
            == end_state
        )
    ...
    def stack_objects(max_vel=1, start_info=None, end_info=None):
        obj_uids = set(
            obj["uid"] for obj in start_info["scene_info"]["movable_objects"].values()
        )
    ...
    def unstack_objects(max_vel=1, start_info=None, end_info=None):
        ...
        for obj_name in start_info["scene_info"]["movable_objects"]:
            ...
            if (
                ...
                # object velocity may not exceed max_vel for successful stack
                if np.all(
                    np.abs(obj_start_info["current_lin_vel"]) < max_vel
                ) and np.all(np.abs(obj_start_info["current_ang_vel"]) < max_vel):
                    return True
        return False
